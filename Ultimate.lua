    ...
    task.spawn(function()
        ...
        while Toggles.Killaura.Value and aw do
            ...
            if alive() and a5 then
                task.spawn(function()
                    if not fY then
                        ...
                        if Toggles.PerkSwitcher.Value and cd[b3].Offhand then
                            local f_ = OffhandPerksActive()
                            if aa and not f_ then
                                SwitchOffhandPerks(true)
                            end
                            if not aa and f_ then
                                SwitchOffhandPerks(false)
                            end
                        end
                        local g0 = aZ:GetChildren()[1]
                        if g0 and g0:FindFirstChild('ID') and g0.ID.Value then
                            local g1 = g0.ID.Value;
                            local g2 = Options.mobWepId.Value and #Options.mobWepId.Value > 2 and Options.mobWepId.Value;
                            local g3 = Options.bossWepId.Value and #Options.bossWepId.Value > 2 and
                                           Options.bossWepId.Value;
                            if g2 and not aa and g1 ~= g2 then
                                equipWepWithId(g2, ' Mobs!')
                                task.wait(0.5)
                            end
                            if g3 and aa and g1 ~= g3 then
                                equipWepWithId(g3, ' Bosses!')
                                task.wait(0.5)
                            end
                        end
                        fY = false
                ...
                if not mounted() then
                    for db, g4 in cd[b3].Skills do
                        local g5, g6 = g4.MeleeOnBoss and aa and 'Melee' or g4.Type or cd[b3].Type, g4.Skill;
                        local g7 = g4.MeleeOnBoss and aa and g4.BossRange or g4.Range or cd[b3].Range;
                        local g8, g9 = g4.Cooldown + Options.KillauraDelay.Value, g5 == 'Ranged' and aa;
                        local ga, fN = g9 and a7 or a8 > 0 and a6 or a7, g9 and a9 or a8;
                        if tick() - (g4.LastUsed or 0) >= g8 then
                            if g5 ~= 'Heal' and fN <= g7 and ab.Value > 0 then
                                if g5 == 'Melee' then
                                    bc:FireServer(g6, aO.Position, (ga - aO.Position).Unit, fZ)
                                elseif g5 == 'Ranged' then
                                    bc:FireServer(g6, ga, nil, fZ)
                                elseif g5 == 'Self' then
                                    bc:FireServer(g6, aO.Position, nil, fZ)
                                elseif g5 == 'Remote' then
                                    if g4.Args == 'MobPosition' then
                                        g6:FireServer(a7, nil, nil, fZ)
                                    elseif g4.Args == 'mobTbl' then
                                        g6:FireServer({a5.Parent}, nil, nil, fZ)
                                    else
                                        g6:FireServer(nil, nil, nil, fZ)
                                    end
                                end
                                g4.LastUsed = tick()
                                ae = tick()
                            end
                            if g5 == 'Heal' and aP.Health.Value / aP.MaxHealth.Value < 0.6 then
                                g6:FireServer(g4.Args or nil, nil, nil, fZ)
                                g4.LastUsed = tick()
                            end
                        end
                    end
    ...
    if classCheck('Summoner') and Toggles.Killaura.Value and aw then
        task.spawn(function()
            while Toggles.Killaura.Value do
                ...
                for db, gb in ce:GetChildren() do
                    if gb.PrimaryPart then
                        if a5 then
                            gb.PrimaryPart.CFrame = CFrame.new(a7)
                        end
                        if gb:FindFirstChild('HealthProperties') and gb.HealthProperties:FindFirstChild('Health') and
                            gb.HealthProperties.Health.Value > 0 and gb.HealthProperties.Health.Value /
                            gb.HealthProperties.MaxHealth.Value <= 0.25 then
...
Toggles.autoEquipBestwWep:OnChanged(function(cM)
    ...
    task.spawn(function()
        while Toggles.autoEquipBestwWep.Value do
            ...
            local g0 = aZ:GetChildren()[1]
            if g0 then
                gf = bK:GetItemStats(g0)['Attack'] or 0
            end
            local gj, gk = 0, 0;
            local gl, gm;
            local gn = b0:GetChildren()[1]
            if gn then
                gj = bK:GetItemStats(gn)['Defense'] or 0
            end
            for K, L in bE:GetChildren() do
                if L:FindFirstChild('Level') and L.Level.Value <= aU.Level.Value then
                    local go = bJ[L.Name]
                    local gp, gq = go.Type == 'Weapon', go.Type == 'Armor'
                    local gr, gs = gp and bK:GetItemStats(L)['Attack'], gq and bK:GetItemStats(L)['Defense']
                    if gp and gr and gr > gf and gr > gg then
                        gh = L;
                        gg = gr;
                        gi = go.DisplayKey
                    end
                    if gq and gs and gs > gj and gs > gk then
                        gl = L;
                        gk = gs;
                        gm = go.DisplayKey
                    end
            ...
            if gh then
                libNoti('Equipped a T' .. bT:GetItemTier(gh) .. ' ' .. gi .. ' - Power: ' .. gg)
                bL:FireServer(gh, aZ)
            end
            if gl then
                libNoti('Equipped a T' .. bT:GetItemTier(gl) .. ' ' .. gm .. ' - Defense: ' .. gk)
                bL:FireServer(gl, b0)
            end
            task.wait(2)
        end
...
local cb = require(game.ReplicatedStorage:WaitForChild("Shared"):WaitForChild("Effects"))
local gt = cb.RenderDamageNumber;
...
local gu = cs['Settings']:AddRightGroupbox('Menu')
gu:AddButton('Unload', function()
    ch:Unload()
end)
gu:AddLabel('Menu bind'):AddKeyPicker('MenuKeybind', {
    Default = 'LeftAlt',
    NoUI = false,
    Text = 'Menu keybind'
})
